...
function createGetPedidoHandler(deps:any={}) { const banco=deps.prisma??prisma, usuario=deps.getUserFromRequest??getUserFromRequest, limitar=deps.checkRateLimit??checkRateLimit, confirmar=deps.confirmarPedidoPorId??confirmarPedidoPorId, flag=deps.confirmacaoAtiva??(()=>process.env.BLACKCAT_CONFIRMACAO_ATIVA==="true"); return async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  // O comprador vê "em análise" enquanto houver caso pendente — inclusive depois
  // de um estorno de upgrade, cuja recomposição ainda está sendo analisada.
  const caso = await banco.revisaoPagamento.findFirst({ where: { pedidoId: pedido.id, status: "PENDENTE" }, select: { id: true } });
  const emRevisao = Boolean(caso);
  return NextResponse.json(atualizado && { id: atualizado.id, status: atualizado.status, valorCentavos: atualizado.valorCentavos, moeda: atualizado.moeda, expiraEm: atualizado.expiraEm, emRevisao, ...(emRevisao ? { mensagem: MENSAGEM_DE_REVISAO_AO_COMPRADOR } : {}) }, { headers: { "Cache-Control": "no-store" } });
}; }
