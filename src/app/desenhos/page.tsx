...
export default function DesenhoPage() {
  ...
  const load = async (p: number, reset: boolean) => {
    setLoading(true);
    const res = await fetch(`/api/series?tipo=desenho&page=${p}&ordem=nota`);
    const data = await res.json();
    const series = data.series ?? [];
    setItems((prev) => reset ? series : [...prev, ...series]);
    setTotal(data.total);
    if (reset) setHero(series.find((s: any) => s.background) ?? null);
    setLoading(false);
  };
    ...
    <div className="min-h-screen">
      ...
      <div className={`px-4 md:px-8 pb-16 ${hero ? "pt-4" : "pt-20"}`}>
        ...
        <h1 className="text-2xl font-bold text-white mb-6">
          Desenhos <span className="text-zinc-500 text-sm font-normal">{total.toLocaleString()}</span>
        </h1>
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7 xl:grid-cols-8 gap-3">
          {items.map((s) => (
            <ContentCard key={s.id} id={s.id} tipo="desenho" titulo={s.titulo} poster={s.poster} ano={s.ano} nota={s.nota} />
          ))}
        </div>
        {items.length < total && (
