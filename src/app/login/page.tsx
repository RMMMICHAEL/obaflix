  ...
  const handleGoogle = async () => {
    ...
    try {
      const desktop = (window as Window & {
        obaflixDesktop?: {
          isDesktop?: boolean;
          startGoogleLogin?: (callbackUrl: string) => Promise<{ ok?: boolean; error?: string }>;
        };
      }).obaflixDesktop;

      if (desktop?.isDesktop && desktop.startGoogleLogin) {
        const result = await desktop.startGoogleLogin(callbackUrl);
        if (!result?.ok) setErro(result?.error || "Não foi possível abrir o login com Google.");
        return;
      }

      await signIn("google", { callbackUrl });
    } catch {
      setErro("Não foi possível abrir o login com Google.");
    } finally {
      setLoading(false);
    }
  };
