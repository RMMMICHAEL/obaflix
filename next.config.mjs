...
const nextConfig = {
  ...
  async headers() {
    ...
    // NAO adicionar `Cache-Control` de pagina aqui. Foi tentado para /filme e
    // /serie e nao funciona na Vercel: o header do next.config vence em
    // `next start`, mas em producao a resposta volta `private, no-cache,
    // no-store` com `x-vercel-cache: MISS` em toda requisicao — medido. Numa
    // pagina, o cache de CDN da Vercel e o proprio ISR (`revalidate`).
    return [
